...
import { CaseData, CaseOutcome, DocumentItem, ConnectionItem, OutcomeItem } from './types';
...
export default function App() {
  ...
  useEffect(() => {
    const initData = async () => {
      const allCases = await relayApi.listCases();
      const allDocs = await relayApi.getDocuments();
      const allConns = await relayApi.getConnections();
      const outcome = await relayApi.getCaseOutcome('1048');
  ...
  const activeCaseId = (routeInfo.type === 'mission_control' || routeInfo.type === 'outcome') 
    ? (routeInfo.caseId || '1048') 
    : '1048';
  const currentCase = cases.find(c => c.id === activeCaseId) || cases[0] || null;
  ...
  const handleLaunchExecution = async (objectives: string[]) => {
    const newCase = await relayApi.createCase({
  ...
  const handleApproveAction = async (caseId: string, approvalId: string) => {
    const updated = await relayApi.approveAction(caseId, approvalId);
    setCases(prev => prev.map(c => (c.id === caseId ? updated : c)));
  };

  const handlePauseCase = async (caseId: string) => {
    const updated = await relayApi.pauseCase(caseId);
    setCases(prev => prev.map(c => (c.id === caseId ? updated : c)));
  };

  const handleResumeCase = async (caseId: string) => {
    const updated = await relayApi.resumeCase(caseId);
    setCases(prev => prev.map(c => (c.id === caseId ? updated : c)));
  };
